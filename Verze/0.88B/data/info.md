...
6. CMD:
    - CMD aplikace, do které vstoupíte když zavřete okno a namísto exit nebo res vepíšete 'cmd'. Nebo když v Rozšířených možnostech zmáčknete cmd.
    - Na příkazu help máte list příkazů kdybyste je potřebovali znovu. Cmd vyžaduje aby vlákno 5 běželo.

    - Příkazy:
        - echo -> echo <cokoli>
        - get -> get version; get var <proměna>; get credits; get file <soubor>
        - stop -> stop <vlákno>
        - start -> start <vlákno>
        - set -> set var:<proměna> to <hodnota>; set event:<název_eventu> to [set/clear]
        - exit -> exit cmd <zpráva>; exit app sdp=[True/False]; exit thread  (spd = shutdownProperly)
        - return -> return <zpráva>
        - clear -> clear [all/interactions]  (all - všechno; interactions tvé interakce)
        - wait -> wait <počet_sekund>

    - Exit kódy:
        - exit0 <- exit cmd
        - exit1 <- exit cmd in couse of error
        - exit2 <- exit app sdp=True
        - exit3 <- exit app sdp=False
        - exit4 <- unuse
        - exit5 <- exit thread 5

6.1 Troubleshooting:
    - Pokud cmd neodpoví, tak se jedná o chybu na druhé straně
    - Pokud nejde vlákno ukončit, tak se podívejte jak pracuje a podle toho jej ukončete
    - Pokud na get var <proměna> kód vrátí <proměna>, tak nejspíš není globálně definovaná a nepůjde to (chci to pak fixnout)
    - Pokud uvidíte špatný exit kód, tak se může jednat o chybu kdekoli
