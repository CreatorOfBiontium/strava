""" Strava
Kód, který objednává 2 na oběd.
Všechno je v souboru info.md ve složce data/
Případně více info je na https://github.com/CreatorOfBiontium/strava
Mail na kterém je github: johnsonpe83@gmail.com
"""
...
try:
    ...
    with open("data/config.json", "r", encoding="utf-8") as confSoubor:
        ...
        showMode4Alert = config["data"]["showMode4Alert"]
        showReleaseNotes = config["data"]["showReleaseNotes"]
...
def checkLP():
    ...
    # Bylo to strašně otravné
    #
    #if radky[1].replace("lastOrder: lastPRlastOrder: ", "") != lastOrder and radky[1].replace("lastOrder: lastPRlastOrder: ", "") != "":
...
if setup == True:
    ...
    def cont():
        ...
        def cont2(encr, jmeno, heslo):
            ...
            if encr == "Ano":
                info["encryptPwd"] = True
            else:
                info["encryptPwd"] = False
                    ...
                    def finish():
                        ...
                        with open(settingsFile, "r+", encoding="utf-8") as settFile:
                            ...
                            if info["encryptPwd"] == True:
                                ...
                                pwdd = Fernet(keyy).encrypt(str(info["pwd"]).encode()).decode()
...
def changeToRm():
    ...
    if allowRM:
        ...
        def lpp():
            toplevel_lp = tk.Toplevel()
            toplevel_lp.title("Nastavení proměn programu")
            
            labelKP  = tk.Label(toplevel_lp, text="Tato funkce do budoucna nemusí být podporována.")
            labelKP.pack()
...
def vlakno1():
    ...
    def insidewrapper():
        ...
        if jmeno != "" and heslo != "" and freeze == False:
            ...
            if odpoved.status_code != 200:
                messagebox.showerror("Špatné přihlašovací údaje", "Kód se pokusil objednat kód se špatnýma přihlašovacíma údajema, nebo došlo k nějaké jiné chybě")
                return
            ...
            odpoved2 = requests.post('https://app.strava.cz/api/pridejJidloS5', data=json_data, headers={'Content-Type': 'text/plain'})
            ...
            if odpoved.status_code == 200 and odpoved2.status_code == 200:
                if lo != lastOrder:
                ...
                if showErrors:
                    time.sleep(1)
                    messagebox.showwarning("Chyba při objednávání", "Nepodařilo se obědnat oběd, zkontrolujte debug.txt pro více info. Nejspíš už není možnost 2 volná.")
                    writeToDebug(f"Error11: Couldn't order food: o: {odpoved.status_code} o2: {odpoved2.status_code}")
...
def startGUI():    
    ...
    try:
        if 3 in threadsjson:
            ...
            menu.add_command(label="Přehled", command=changeToPrehled)
            menu.add_command(label="Nastavení", command=changeToNastaveni)
            menu.add_command(label="Chat GPT", command=changeToCgpt)
            menu.add_command(label="Rozšířené možnosti", command=changeToRm)
            ...
            if showReleaseNotes:
                release_notes_toplevel = tk.Toplevel(root)
                release_notes_toplevel.title(f"Poznámky k vydání - strava {VERSION}")
                release_notes_toplevel.geometry("370x420")
                release_notes_toplevel.minsize(320, 400)
                release_notes_toplevel.maxsize(400, 500)
                
                nadpis = tk.Label(release_notes_toplevel, text=f"Strava {VERSION}", font=Font(family="Arial", size=30))
                nadpis.pack()
                
                novinky = tk.Label(release_notes_toplevel, text=f"\nVerze 0.9 je tvořena převážne z oprav chyb, ale i pár dalších novinek. Seznam naleznete níže. Je možné, že se zde pořád nějaké chyby nachází, v tom případě mi je ideálně nějak sdělte. Teď to bude poslední velká aktualizace, protože mám v plánu se věnovat něčemu trochu jinému, ale pořád jakmile přijdu na chybu se ji pokusím opravit.\n\n\n- Úprava velikosti okna\n- Úprava kontroly kódu\n- Oprava přepínání \"scén\" na Linuxech\n- Částečná oprava restartování na Linuxech\n- Vyčištění kódu\n- Přidání poznámek k vydání\n- Chybové ohlášení když oběd nelze objednat (Nejspíš kvůli tomu, že je už zabraný)\n- Příprava pro budoucí pluginy\n\nTo je z tohoto updaty vše, jak jsem už zmínil, delší dobu nečekejte updaty.",
                                   wraplength=320, anchor="w", justify="left")
                novinky.pack()
                
                with open("data/config.json", "r+") as configFile:
                    jsonData = json.load(configFile)
                    jsonData["data"]["showReleaseNotes"] = False
                    jsonDone = json.dumps(jsonData, indent=4)
                    configFile.seek(0)
                    configFile.write(jsonDone)
                    configFile.truncate()
